...
/*   Updated: 2025/01/26 03:08:42 by asagymba         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/**
 * Draws a ceiling and floor (defined in \p data) to "buffer" in \p data.
...
static void	ft_draw_ceiling_and_floor(struct s_data *data)
{
	...
	i = 0;
	while (i < data->buffer.height / 2)
	{
		j = 0;
		while (j < data->buffer.width)
			ft_pixel_put_on_image(&data->buffer,
				j++, i, &data->colors[COLOR_CEILING]);
		i++;
	}
	while (i < data->buffer.height)
	{
		j = 0;
		while (j < data->buffer.width)
			ft_pixel_put_on_image(&data->buffer,
				j++, i, &data->colors[COLOR_FLOOR]);
		i++;
	}
}

/**
 * Prints a pixel in ft_draw_line().
 * @param	arg	ft_draw_wall_pixel() data.
 */
 ...
 */
static void	ft_draw_line(struct s_data *data, struct s_ray *ray, int x)
{
	...
	{
		ft_draw_line_pixel(&data->buffer, ray, &draw_line_data, x);
		draw_line_data.i++;
	}
}

/**
 * Draws walls (use textures from \p data) to "buffer" in \p data..
 * @param	data	cub3D's data.
 */
static void	ft_draw_walls(struct s_data *data)
{
...
{
	if (data->buffer.img == NULL)
	{
		data->buffer.img = mlx_new_image(data->conn, WIN_X, WIN_Y);
		if (data->buffer.img == NULL)
		{
			if (errno == 0)
				errno = EACCES;
		...
		data->buffer.width = WIN_X;
		data->buffer.height = WIN_Y;
		data->buffer.img_pixels = mlx_get_data_addr(data->buffer.img,
				&data->buffer.bits_per_pixel, &data->buffer.size_line,
				&data->buffer.endianness);
	}
	ft_draw_ceiling_and_floor(data);
	ft_draw_walls(data);
	(void)mlx_put_image_to_window(data->conn, data->win, data->buffer.img,
		BEGINNING, BEGINNING);
	/* REMOVE THIS! */
#include <sys/time.h>
	struct timeval	time;
